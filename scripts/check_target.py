...
def main(args):
    ...
    dataOriginal = dataCleaner.fit_transform(data)

    for label in dataOriginal.columns:
        target = dataOriginal[[label]]
        data = dataOriginal.drop(columns=[label])
        ...
        # # Save the PCA plot
        # plt.savefig(f"../output/exploreTarget/{label}_pca.png")
        ...
        # Save the UMAP plot
        plt.savefig(f"../output/exploreTarget/{label}_umap.png")
        plt.close()  # Close the plot to free memory
